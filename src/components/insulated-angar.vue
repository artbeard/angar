<template lang="">
    <div class="container">
		...
		<form v-else @submit.prevent="submitHandle(v$.$validate())" class="row" v-show="calculatorVisibility">
            ...
            <div class="col-12 col-lg-6 mb-3">
				...
				<InputText id="thicknessInsulation" class="w-100"
				   ...
				   type="number"
				   :class="{'p-invalid' : submitted && v$.thicknessInsulation.$invalid }"
				/>
				<span v-if="submitted && (v$.thicknessInsulation.$error || v$.thicknessInsulation.$invalid)">
					<span id="length-error" v-for="(error, index) of v$.thicknessInsulation.$silentErrors" :key="index">
						<small class="p-error">{{error.$message}}</small>
					</span>
				</span>
			</div>
        ...
        <div class="row" v-show="!calculatorVisibility">
			...
			<div class="col-12">
				<div class="container-fluid calculatedDataInsulated fs-6">
					<p class="text-center">Бескаркасный ангар утепленный, {{S}} м<sup>2</sup> ({{length}}м x {{width}}м)</p>
					<table class="table table-borderless">
						<tbody>
						<tr>
							<td>
								<img src="../assets/img/insulated-angar-schema.jpg" height="160" alt="">
							</td>
...
export default defineComponent({
	...
	mixins: [printMixin],
	data: ()=>({
		printTitle: 'Коммерческое предлоежение от ООО Сибангар - Ангар бескаркасный утепленный',
	}),
})
</script>
<script setup>
